...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!SessionManager::verifyCsrf($_POST['csrf_token'] ?? '')) {
        ...
        $oldOrg   = htmlspecialchars($orgName, ENT_QUOTES, 'UTF-8');

        $validator = new Validator();
        if (!$validator->validateRegisterInput([
            'email'            => $email,
            'password'         => $password,
            ...
            $result = $auth->register($email, $password, $orgName);
            ...
            // Fehler von AuthService (z. B. E-Mail bereits vergeben)
...
<head>
    ...
    <title>Registrieren – ReviewReplyPro</title>
...
<div class="auth-wrap">
    <div class="auth-card">
        ...
        <div class="auth-title">Kostenloses Konto erstellen</div>
        ...
        <form method="POST" action="/register.php">
            ...
            <div class="auth-field">
                <label for="org_name">Unternehmensname</label>
                <input type="text" id="org_name" name="org_name"
                       value="<?= $oldOrg ?>"
                       placeholder="z. B. Restaurant Zum Stern"
